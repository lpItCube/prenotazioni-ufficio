...
const CalendarElement: React.FC<CalendarProps> = (props): JSX.Element => {
	...
	useEffect(() => {
		const currentDate: any = document.querySelector(
				".calendar-tool__current-date"
			),
			daysTag: any = document.querySelector(".days"),
			prevNextIcon: any = document.querySelectorAll(
				".calendar-tool__icons svg"
			);
		...
		const renderCalendar = () => {
			...
			const daysEl: any = document.querySelectorAll(
				".days li:not(.inactive)"
			);
	...
	return (
		<div
			className={`calendar-tool${openCalendar ? " is-open" : ""}`}
			ref={ref}
		>
