...
const GridCreate: React.FC<GridCreateProps> = (props): JSX.Element => {
	...
	useEffect(() => {
		if (room) {
			setUpdateGrid(
				room.gridPoints && room.gridPoints.length > 0
					? room.gridPoints
					: []
			);
		...
		if (updateGrid) {
			for (let y = 0; y < ySize; y++) {
				...
				for (let x = 0; x < xSize; x++) {
					const point = updateGrid.find(
						(p: GridPoint) => p.x === x && p.y === y
					);
	...
	useEffect(() => {
		const returnElement = (
			...
			const element: GridPoint | undefined = grid
				?.flat()
				.find((el: any) => el.x === findX && el.y === findY);
			return element;
		};
			...
			if (
				...
				if (event.key === "ArrowRight") {
					setSelectedCell((prev: GridPoint) => ({
						...
						info: returnElement(
							prev.x !== xSize - 1 ? prev.x + 1 : xSize - 1,
							prev.y
						)?.info,
						seatName: returnElement(
							prev.x !== xSize - 1 ? prev.x + 1 : xSize - 1,
				...
				if (event.key === "ArrowDown") {
					setSelectedCell((prev: GridPoint) => ({
						...
						info: returnElement(
							prev.x,
							prev.y !== ySize - 1 ? prev.y + 1 : ySize - 1
						)?.info,
						seatName: returnElement(
							prev.x,
							prev.y !== ySize - 1 ? prev.y + 1 : ySize - 1
	...
	return (
		<>
			<div className="creation-table__wrapper">
				<div
					className="creation-table__body"
					id="isometric-container"
					style={{ width: xSize * 50 + "px" }}
				>
