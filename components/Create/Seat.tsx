...
const Seat: React.FC<SeatProps> = (props): JSX.Element => {
	...
	const [seatProps, setSeatProps] = useState<{ canvasClass: string }>({
		canvasClass: "",
	});
	...
	const handleDeleteSingleSeat = (cell: GridPoint) => {
		...
		const filtred = reserves.filter(
			(res: Reserve) =>
				res?.seat?.name === cell.seatName || res.seat?.type === WHOLE
		);
	...
	useEffect(() => {
		...
		const yourReserveInRoom = reserves.find(
			(r: Reserve) => r.user.username === username
		);
		const seatReserve = reserves.find(
			(r: Reserve) => r.seat?.name === cell.seatName
		);
		const yourReserve = seatReserve?.user.username === username;
		const wholeRoom = reserves.find((r: Reserve) => r.seat?.type === WHOLE);
		const isPending = reserves.find(
			(r: Reserve) =>
				r.status === PENDING && r.seat?.name === cell.seatName
		);

		if (wholeRoom) {
			const isYour = reserves.some(
				(r: Reserve) =>
					r.user.username === username &&
					r.seat?.name.includes("whole")
			);
			const isPending = reserves.some(
				(r: Reserve) => r.status === PENDING
			);
			if (isYour) {
			...
			if (yourReserveInRoom && !isAdmin && !yourReserve) {
				setSeatProps((prev) => ({
					canvasClass: `${prev.canvasClass} not-available`,
				}));
			}
			if (yourReserve) {
				setSeatProps({
					canvasClass: `${
						isPending ? "pending" : "your"
					} clickable del`,
				});
			}
		}
		...
		<div className="creation-options__seat-container">
			<div
				...
				onMouseOver={() =>
					dispatch(
						setUserOnSeat({
							userOnSeat: reservesInDay.filter(
								(res: Reserve) =>
									res.seat?.name === cell.seatName ||
									res.seat?.type === WHOLE
							),
						})
					)
