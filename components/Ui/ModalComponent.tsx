...
const ModalComponent: React.FC<ModalComponentProps> = (props): JSX.Element => {
	...
	return (
		<div
			className={`modal${
				modalStatus && modalType === refType ? " show-modal" : ""
			}`}
		>
			<div
				onClick={() => handleCloseModal()}
				className={`modal__obscurer${
					modalStatus && modalType === refType ? " active" : ""
				}`}
			></div>
			<div className="modal__content">
				<div className="modal__header">
					<div className="modal__title">
						<h5 className="semiBold">{modalTitle}</h5>
					...
					>
						<IoClose size={32} color={Colors.light700} />
					</div>
				</div>
