...
const Room: React.FC = (): JSX.Element => {
	...
	const [selectedDomain, setSelectedDomain] =
		useState<OptionItem>(DEFAULT_DOMAIN_VALUE);
	const [selectedOffice, setSelectedOffice] =
		useState<OptionItem>(DEFAULT_OFFICE_VALUE);
	const [selectedRoom, setSelectedRoom] =
		useState<OptionItem>(DEFAULT_ROOM_VALUE);
	...
	useEffect(() => {
		const trueCount: number = [
			Boolean(selectedDomain.value),
			Boolean(selectedOffice.value),
			Boolean(selectedRoom.value),
	...
	useEffect(() => {
		const domain = domains.find(
			(domain) => domain.id === selectedDomain.value
		);
		if (
			domain &&
			domain.hasOwnProperty("office") &&
			domain.office.length > 0
		) {
	...
	const handleCreation = async (type: number) => {
		if (type === StepperState.DOMAIN) {
			const res: AxiosResponse<Domain> = await axios.post("/api/domain", {
				name: createName,
			});
			setSelectedDomain({ value: res.data.id, label: res.data.name });
			setDomains([...domains, res.data]);
		} else if (type === StepperState.OFFICE) {
			const res: AxiosResponse<Office> = await axios.post("/api/office", {
				name: createName,
			...
			const res: AxiosResponse<Room> = await axios.post("/api/room", {
				name: createName,
