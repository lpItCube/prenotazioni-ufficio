...
const Prenotazioni: React.FC = (): JSX.Element => {
	...
	const [filterMode, setFilterMode] = useState<OptionItem>({
		label: "Le mie prenotazioni",
		value: "myUser",
	});
	const [filterRoom, setFilterRoom] = useState<OptionItem>({
		label: "Tutte le stanze",
		value: "",
	});
	const [filterDay, setFilterDay] = useState<OptionItem>({
		label: "Tutte le date",
		value: "",
	});
	const [showFilters, setShowFilters] = useState<boolean>(false);
	const [hitDeleteButton, setHitDeleteButton] = useState<HitModalButton>({
		loading: false,
		id: null,
	});
	...
	useEffect(() => {
		const getReserves = async (method: string) => {
			const response: AxiosResponse<Reserve[]> = await axios.get(
				`/api/reserve/currentUser?currentUser=${userId}&method=${method}`
			);
			...
			if (filterDay.value !== "") {
				sortedResponse = sortedResponse.filter(
					(res: Reserve) =>
						new Date(res.from).toDateString() === filterDay.value
				);
			}

			const reorderData = sortedResponse.sort((a: any, b: any) =>
				a.to > b.to ? 1 : -1
			);
				...
				<TableRow key={"not-found"} className="">
					<TableCol className="prenotazioni__empty">
						...
						<p className="semiBold">
							{filterMode.label} {filterRoom.label}{" "}
							{filterDay.label}
						</p>
					</TableCol>
				</TableRow>
			...
			<div className="prenotazioni__container">
				...
				<div className="prenotazioni__wrapper">
					<div className="prenotazioni__header">
						<h2 className="table__title txt-h3">Prenotazioni</h2>
						<Button
							type="button"
